...
public class GenerationalElitistGA<T extends AbstractSolution> implements IAlgorithm<T> {
    ...
    public T run() {
        ...
        while (!conditionsSatisfied(generation, population)) {
            ...
            while (newPop.size() < populationSize) {
                Pair<T> parents = getParents(population);
                Pair<T> children = crossoverOperator.getChildren(parents, RANDOM);
                mutate(children);
    ...
    private void mutate(Pair<T> children) {
    ...
    private Pair<T> getParents(Population<T> population) {
        T first = selection.select(population, RANDOM);
        T second = selection.select(population, RANDOM);
